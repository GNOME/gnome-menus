...
get_by_category_func (EntryDirectory  *ed,
                      ...
                      gpointer         user_data)
{
  const char *category = user_data;

  if (ed->is_legacy && !desktop_entry_has_categories (entry))
    {
      if (strcmp (category, "Legacy") != 0)
        return TRUE;
    ...
    {
      if (!desktop_entry_has_category (entry, category))
        return TRUE;
                                      ...
                                      DesktopEntrySet    *set)
{
  ...
  entry_directory_list_add (list,
                            get_by_category_func,
                            set,
                            (char *) category);
}
...
get_inverse_func (EntryDirectory  *ed,
                  ...
                  gpointer         user_data)
{
  DesktopEntrySet *inverse = user_data;
  ...
  desktop_entry_set_add_entry (inverse,
                               entry,
                               file_id ? file_id : relative_path);
  desktop_entry_unref (entry);
...
entry_directory_list_invert_set (EntryDirectoryList *list,
                                 DesktopEntrySet    *set)
{
  DesktopEntrySet *inverse;

  menu_verbose (" Inverting set %p relative to list %p\n",
                set, list);

  inverse = desktop_entry_set_new ();
  entry_directory_list_add (list,
                            get_inverse_func,
                            set,
                            inverse);

  desktop_entry_set_swap_contents (set, inverse);

  desktop_entry_set_unref (inverse);
}
